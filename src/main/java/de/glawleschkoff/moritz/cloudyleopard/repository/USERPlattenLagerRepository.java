...
public interface USERPlattenLagerRepository extends JpaRepository<USERPlattenLagerModel, Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value = "update USERPlattenLager u set u.LagerPlatz = :lagerPlatz, u.Lng = :lng, u.Brt = :brt, u.MZ3 = :mz3, u.MZ6 = :auslagerId, u.MZ1 = :auslagerInfo+' '+CONVERT(DAYOFMONTH(CURRENT_TIMESTAMP()),SQL_VARCHAR)+'.'+CONVERT(MONTH(CURRENT_TIMESTAMP()),SQL_VARCHAR)+'.'+CONVERT(YEAR(CURRENT_TIMESTAMP()),SQL_VARCHAR)+' '+CONVERT(CURTIME(),SQL_VARCHAR), u.MZD1 = CURRENT_DATE(), u.Menge = :menge where u.PlattenId = :plattenId", nativeQuery = true)
    void updateUSERPlattenLager(String lagerPlatz, Double lng, Double brt, String mz3, Double plattenId, String auslagerId, String auslagerInfo, Double menge);

    @Transactional
    @Modifying
    @Query(value = "update USERPlattenLager u set u.LagerPlatz = :lagerPlatz, u.Lng = :lng, u.Brt = :brt, u.MZ3 = :mz3, u.MZ6 = :auslagerId, u.MZ1 = '', u.MZD1 = NULL, u.Menge = :menge where u.PlattenId = :plattenId", nativeQuery = true)
    void updateUSERPlattenLagerBearbeiten(String lagerPlatz, Double lng, Double brt, String mz3, Double plattenId, String auslagerId, Double menge);
